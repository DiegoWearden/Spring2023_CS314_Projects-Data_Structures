...
public class Graph {
    ...
    public void dijkstra(String startName) {
        ...
        clearAll();
        currentStartVertexName = startName;
        // Initialize the priority queue and set the starting vertex's cost to 0
        ...
        while (!pq.isEmpty()) {
            ...
            Vertex currVertex = currPath.dest;
            if(currVertex.scratch == 0){
                currVertex.scratch = 1;
            }
            for(Edge e : currVertex.adjacent){
                Vertex neighbor = e.dest;
                double costToNeighbor = currVertex.weightedCostFromStartVertex + e.cost;
                if (costToNeighbor < neighbor.weightedCostFromStartVertex) {
                    neighbor.weightedCostFromStartVertex = costToNeighbor;
                    neighbor.prev = currVertex;
                    pq.add(new Path(neighbor, costToNeighbor));
                }
            }
        }
    }
    ...
    public void findAllPaths(boolean weighted) {
        ...
        for (Vertex start : vertices.values()) {
            ...
            // Loop through all other vertices that are in graph
            for(Vertex other : vertices.values()){
                if (other != start && other.prev != null){
                    start.numVertexConnected++;
                    start.totalWeightedPathLength += other.weightedCostFromStartVertex;
                    start.totalUnweightedPathLength += other.numEdgesFromStartVertex;
                    if (other.weightedCostFromStartVertex > maxCost){
                        maxCost = other.weightedCostFromStartVertex;
