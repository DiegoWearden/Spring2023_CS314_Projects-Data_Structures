...
public class FootballRanker {
    ...
    private double printRootMeanSquareError(List<String> humanRanks, TreeSet<AllPathsInfo> paths, boolean showResults) {
        ...
        double squaredRankDifferencesSum = 0;

        for (int i = 0; i < humanRanks.size(); i++) {
            String teamName = humanRanks.get(i);
            int actualRank = i + 1;

            int predictedRank = paths.size() + 1;
            int counter = 0;
            boolean found = false;
            Iterator<AllPathsInfo> iterator = paths.iterator();
            while (iterator.hasNext() && !found) {
                AllPathsInfo pathInfo = iterator.next();
                counter++;
                if (pathInfo.getName().equals(teamName)) {
                    predictedRank = counter;
                    found = true;
                }
            }

            if (showResults){
                System.out.printf("%-" + PADDING + "s - actual rank: %-1d predicted rank: %-1d\n", teamName, actualRank, predictedRank);
            }
            double rankDifference = actualRank - predictedRank;
            squaredRankDifferencesSum +=rankDifference * rankDifference;
        }

        double meanSquaredError = squaredRankDifferencesSum / humanRanks.size();
        double rootMeanSquareError = Math.sqrt(meanSquaredError);
        double roundedRootMeanSquareError = Math.round(rootMeanSquareError * roundingFactor) / roundingFactor;

        if (showResults) {
            System.out.println("Root Mean Square Error: " + ourFormatter.format(rootMeanSquareError));
        }

        return roundedRootMeanSquareError;
    }
